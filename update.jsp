...
//執行 SQL 指令	          
				if(session.getAttribute("name") != null ){
					...
					sql = "UPDATE `member` SET `name`='"+request.getParameter("name")+"', `password`='"+request.getParameter("password")+"' WHERE `name`='"+session.getAttribute("name")+"'";
					con.createStatement().execute(sql);	
					con.close();//結束資料庫連結
					session.setAttribute("name",request.getParameter("name"));
					out.print("<script>alert('更新成功！');location.href='member.jsp'</script>");
					}
					else{
					  con.close();//結束資料庫連結	
					  out.print("<script>alert('更新失敗！');location.href='member.jsp'</script>");
					}
				}
				else{
					out.print("<script>alert('尚未登入！');location.href='login.jsp'</script>");
					con.close();//結束資料庫連結
				}
		}
	}
