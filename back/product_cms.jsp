...
try {
    ...
    try {
           ...
           out.println("共"+total_content+"筆產品<p>");
		   %>
		   
		   <p><a href='product_add.jsp'>新增其他產品(+)</a></p>  
           ...
           //使用超連結方式, 呼叫自己, 使用get方式傳遞參數(變數名稱為page)
				for(int i=1;i<=page_num;i++) 
					out.print("<a href='product_cms.jsp?page="+i+"'>"+i+"</a>&nbsp;");
				out.println("<p>");
           //讀取page變數
				...
				if(current_page==0) //若未指定page, 令current_page為1
					current_page=1;
				if(current_page!=1) 
					out.print("<a href='product_cms.jsp?page=1'>"+"第一頁</a>&nbsp;"); //&nbsp;html的空白
				if(current_page>1) 
					out.print("<a href='product_cms.jsp?page="+(current_page-1)+"'>"+"上一頁</a>&nbsp;"); //&nbsp;html的空白
				if(current_page<page_num){
					out.print("<a href='product_cms.jsp?page="+(current_page+1)+"'>"+"下一頁</a>&nbsp;"); //&nbsp;html的空白
					out.print("<a href='product_cms.jsp?page="+page_num+"'>"+"最後一頁</a>&nbsp;"); //&nbsp;html的空白
				}
				if(current_page>page_num)
					current_page=1;
				out.println("共"+page_num+"頁，目前在第"+current_page+"頁");
				out.println("<form name='f' action='product_cms.jsp' method='POST'>跳至<input type='text' size='3' name='page' value=1>頁<input type='submit' value='送出'>");
				out.println("</form><hr>");
	       //計算開始記錄位置   
...
//  逐筆顯示, 直到沒有資料(最多還是5筆)
           while(rs.next())
                {
                 out.println("產品編號："+rs.getString(1)+"<br>");
                 out.println("產品名稱："+rs.getString(2)+"<br>");
                 out.println("產品說明："+rs.getString(3)+"<br>");
                 out.println("產品價格："+rs.getString(4)+"<br>");
                 out.println("產品類型："+rs.getString(5)+"<br>");
				 out.println("尺寸："+rs.getString(6)+"<br>");
				 out.println("顏色："+rs.getString(7)+"<br>");
				 out.println("數量："+rs.getString(8)+"<br>");
				 out.println("狀態(on上架/off下架)："+rs.getString(9)+"<br>");
				 out.println("<p><a href='product_update.jsp?pid="+ rs.getString(1) +"'>修改產品</a>  <a href='product_delete.jsp?pid="+ rs.getString(1) +"'>下架產品</a>  <a href='product_on.jsp?pid="+ rs.getString(1) +"'>上架產品</a></p><hr>") ;
