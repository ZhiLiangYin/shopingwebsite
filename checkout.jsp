...
<body>
  <header>
    ...
    <div class="row">
        <div class="col-75">
          <div class="container">
            <form action="order.jsp" method="POST">
				<input type="hidden" name="do_order" value="go" />
              <div class="row">
                <div class="col-50">
                  <h3>收貨人資訊</h3>
                  <label for="name"><i class="fa fa-user"></i> 姓名</label>
                  <input type="text" id="name" name="name" placeholder="張大壯" required>
				  <label for="tel"> 電話</label>
                  <input type="text" name="tel" placeholder="09××××××××(限數字)" required>
                </div>
      
                <div class="col-50">
                  <h3>付款</h3>
				  <label>
					<input type="radio" name="sameadr" value="宅配"> 刷卡付款宅配到府
					<input type="radio" checked="checked"  name="sameadr" value="超取"> 超商取貨付款
				  </label>
                  ...
                  <label for="expmonth">卡片有效期限(月-年)</label>
                  <input type="text" id="expmonth" name="expmonth" placeholder="1123">
				  ...
				  <label for="adr"><i class="fa fa-address-card-o"></i>收貨地址/超商店址<input type="button" value="選擇超商店址" onclick="this.form.address.value='便利超商中原店'" ></label>
				  
                  <input type="text" id="adr" name="address" placeholder="台北市大安區建國南路二段" required>
                </div>
		...
		<tr><th width=350><div class="goods">商品</div></th><th><div class="goods">尺寸</div></th><th><div class="goods">顏色</div></th><th><div class="price">單價</div></th><th><div class="number">數量</div></th><th><div class="subtotal">總金額</div></th><th><div class="goods">庫存</div></th><th><div class="goods">上架狀態</div></th></tr>
        <!--商品内容-->
		...
		if(session.getAttribute("mid")!=null){
			...
			//執行 SQL 指令	          
							...
							sql="SELECT pid,price,amount,total,name,size,color,count,status FROM cart_view WHERE `status`='on' AND mid="+umid+"  ORDER by pid";
		...
		%>
			...
			<br>
			 <input type="submit" value="下一步" class="btn" <%if(total==0)out.print("disabled='disabled'");%>>
            </form>
        </div>
            </h4>
            
            <hr>
            <p>Total <span class="price" style="color:red"><b>$<%=total%></b></span></p>
          </div>
        </div>
    </div>
    <footer>
