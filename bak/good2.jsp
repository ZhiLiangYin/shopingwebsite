...
<body>
    ...
    <section>
      <div class="section-image">
            <img src="image/pant.jpg" width="300">
      </div>
      <div class="title">
<%
	...
	//sql="SELECT `count` FROM `product` WHERE `size`='"+size+"' AND color='"+color+"' AND `type`='2'";
	...
	//執行 SQL 指令
				...
				sql="SELECT count(`msg_type`) FROM `message` WHERE msg_type=3" ;
				...
				for(int i=1;i<=page_num;i++) 
					out.print("<a href='good2.jsp?page="+i+"'>"+i+"</a>&nbsp;");
				out.println("<p>");
				...
				if(current_page==0) //若未指定page, 令current_page為1
					current_page=1;
				if(current_page!=1) 
					out.print("<a href='good2.jsp?page=1'>"+"第一頁</a>&nbsp;"); //&nbsp;html的空白
				if(current_page>1) 
					out.print("<a href='good2.jsp?page="+(current_page-1)+"'>"+"上一頁</a>&nbsp;"); //&nbsp;html的空白
				if(current_page<page_num){
					out.print("<a href='good2.jsp?page="+(current_page+1)+"'>"+"下一頁</a>&nbsp;"); //&nbsp;html的空白
					out.print("<a href='good2.jsp?page="+page_num+"'>"+"最後一頁</a>&nbsp;"); //&nbsp;html的空白
				}
				if(current_page>page_num)
					current_page=1;
				out.println("共"+page_num+"頁，目前在第"+current_page+"頁");
				out.println("<form name='f' action='good2.jsp' method='POST'>跳至<input type='text' size='3' name='page' value=1>頁<input type='submit' value='送出'>");
				out.println("</form><hr>");
	...
	//顯示結果 
				...
				//遞減排序, 讓最新的資料排在最前面
				sql="SELECT * FROM `message` WHERE `msg_type`=3 ORDER BY `msg_id` DESC LIMIT ";
				sql+=start_record+",5";
				rs=con.createStatement().executeQuery(sql);
	//逐筆顯示, 直到沒有資料(最多還是5筆)
				while(rs.next())
