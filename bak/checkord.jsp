...
//選擇資料庫   
			if( request.getParameter("neme") != null && !request.getParameter("neme").equals("") &&
			    request.getParameter("tel") != null && !request.getParameter("tel").equals("") && request.getParameter("address") != null && !request.getParameter("address").equals("") && 
				request.getParameter("payway") != null && !request.getParameter("payway").equals("")){ 
                ...
                if(rs.next()) {
					mid = rs.getString(1) ;
					String neme = request.getParameter("neme").toString();    
					String address = request.getParameter("address").toString();
					String tel = request.getParameter("tel").toString();   
					String payway = "\"" ;
					payway += request.getParameter("payway").toString(); 
					payway += "\"" ;
					sql ="SELECT `name`, `price`, `amount` FROM cart_view WHERE `mid` = '"+ mid + "'";
					rs=  con.createStatement().executeQuery(sql);
					String products = "\"" ;
					int total = 0 ;
					while(rs.next()){
						products = products + rs.getString(1) + "<br>" ;
						total = total + Integer.parseInt(rs.getString(2)) * Integer.parseInt(rs.getString(3)) ;
					}
					products = products + "\"" ;
					sql="INSERT `order` (`mid`,`products`,`price`,`pay_way`, `name`, `address`, `tel`) " ;
					sql += "VALUES(" + mid + ", "+ products + ", "+ total + ", " + payway +", " + neme + ", " + address + ", " + tel + ")";
					con.createStatement().executeUpdate(sql);
					sql ="SELECT `pid`,`amount`, `mid` FROM cart_view WHERE `mid` = '"+ mid + "'";
					rs=  con.createStatement().executeQuery(sql);
					while(rs.next()){
				        sql = "SELECT `count` FROM `product` WHERE `pid`= '"+ rs.getString(1) + "'";
				        ResultSet rt=  con.createStatement().executeQuery(sql);	
						rt.next();
						int remain = Integer.parseInt(rt.getString(1)) - Integer.parseInt(rs.getString(2)) ;
						sql = "UPDATE `product` SET `count` =" + remain + " WHERE `pid` = " + rs.getString(1) ;
						con.createStatement().executeUpdate(sql);
					}
						sql = "DELETE FROM `cart` WHERE `mid` ="+ mid ;
						con.createStatement().executeUpdate(sql);
						out.println("訂單已送出<br>購物車已清空<br><a href='index.jsp'>謝謝惠顧!!!</a>");
					
				}
				else out.println("尚未登入");
				
				con.close();
			}
			else {
			    out.print("<a href='checkout.jsp'>訂單輸入資訊不符</a>");
				con.close();
			}
...
%>
			<footer>
                ...
                <div class="brand-area">
                  <a class="brand-logo"><img class="brand-image" src="image/zairito.png" alt="zairito" /></a>
                </div>
                ...
                <div class="footer-li">
                  <ul><div class="footer-font">販售商品</div>
                    <li>休閒褲</li>
                    <li>牛仔褲</li>
                    <li>襯衫</li>
                    <li>帽T</li>
