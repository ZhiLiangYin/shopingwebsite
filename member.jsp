...
//Step 4: 執行 SQL 指令	
			if(session.getAttribute("name") != null ){
				sql = "SELECT * FROM `member` WHERE `name`='" +session.getAttribute("name")+"';"; 
				ResultSet rs =con.createStatement().executeQuery(sql);
...
%>
			...
			※ 訂單紀錄：
			<table border=1>
			<tr><th>訂單日期</th><th>訂單單號(點擊查明細)</th><th>狀態</th><th>申請退貨</th></tr>
...
<%
			...
			sql="SELECT `oid`,`order_date`,`ship_status` FROM `order` WHERE `mid`="+umid+" ORDER BY `order_date`";
			ResultSet rs =con.createStatement().executeQuery(sql);
			String status="";
			while(rs.next()){
				status=rs.getString(3);
			%>
			<tr><td><%=rs.getString(2)%></td><td style="text-align:center;font-size:20;"><a href="order_list.jsp?no=<%=rs.getString(1)%>"><%=rs.getString(1)%></a></td><td style="text-align:center;"><%=status%></td><td style="text-align:center;"><%if(!status.equals("已退貨"))out.print("<a href='order_drop.jsp?no="+rs.getString(1)+"'>退貨</a>");%></td></tr>
			<%
			}
			con.close();//結束資料庫連結
		}
