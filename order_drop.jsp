...
<body>
    <header>
      ...
      <%
		...
		<h1 style="text-align:center;">退貨處裡</h1>
		<h2>
        <%
		...
		String oid=request.getParameter("no");
       
		int i=0;
		String countAry[]= new String[]{"", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", ""};
		try {
		//載入資料庫驅動程式 
			Class.forName("com.mysql.jdbc.Driver");
			try {
		...
		//執行 SQL 指令
						...
						//訂單
						sql="SELECT `order_date`,`name`,`tel`,`pay_way`,`pay_name`,`pay_card`,`address`,`total`,`ship_status` FROM `order` WHERE `mid`="+umid+" AND `oid`="+oid;
						ResultSet rs=  con.createStatement().executeQuery(sql);
						if(rs.next()){
							out.println("<table border=1><tr><td>訂單單號：</td><td>"+oid+"</td></tr>");
							out.println("<tr><td>日期：</td><td>"+rs.getString(1)+"</td></tr>");
							out.println("<tr><td>收貨人：</td><td>"+rs.getString(2)+"</td></tr>");
							out.println("<tr><td>電話：</td><td>"+rs.getString(3)+"</td></tr>");
							out.println("<tr><td>付款方式：</td><td>"+rs.getString(4)+"</td></tr>");
							if(rs.getString(5)!=null)
								out.println("<tr><td>刷卡人：</td><td>"+rs.getString(5)+"</td></tr>");
							if(rs.getString(6)!=null)
								out.println("<tr><td>信用卡號：</td><td>"+rs.getString(6)+"</td></tr>");
							out.println("<tr><td>收件地址：</td><td>"+rs.getString(7)+"</td></tr>");
							out.println("<tr><td>訂單金額：</td><td>"+rs.getString(8)+"</td></tr>");
							out.println("<tr><td>出貨狀態：</td><td>"+rs.getString(9)+"</td></tr></table>");
							
						}
						//查出貨明細
						sql="SELECT `name`,`size`,`color`,`amount`,`price` FROM `order2_view` WHERE `mid`="+umid+" AND `oid`="+oid+" ORDER BY 1,2,3";
						rs=  con.createStatement().executeQuery(sql);
						out.println("<table style='color:blue;' border=1><tr><th>名稱</th><th>尺寸</th><th>顏色</th><th>單價</th><th>數量</th></tr>");
						while(rs.next()){
							...
							out.println("<td style='text-align:right;'>"+rs.getString(5)+"</td></tr>");
							
						}
						out.println("</table><br><hr>");
						//訂單標記已退貨
						sql="UPDATE `order` SET `ship_status`='已退貨' WHERE `mid`="+umid+" AND `oid`="+oid;
						con.createStatement().executeUpdate(sql);
						//轉出貨明細: cart轉ship
						sql="SELECT pid,amount FROM ship WHERE `oid`="+oid;
						rs=  con.createStatement().executeQuery(sql);
						String db_pid="";
						int db_amount=0;//數量
						...
						while(rs.next()){
							...
							db_amount=rs.getInt(2);//數量
							
							if(db_amount>0) {
								countAry[i++]="UPDATE `product` set `count`= (`count`+"+db_amount+") WHERE pid="+db_pid; // 更新庫存SQL
							}
						...
						// 執行更新庫存SQL
						for(int j=0;j<i;j++){
							//out.print(countAry[j]+"<BR>");
							sql=(countAry[j]);
							//out.println("更新庫存: "+sql+"<br>");
							con.createStatement().executeUpdate(sql);
						}
						out.println("<script>alert('已完成退貨！可至會員區確認~');location.href='member.jsp'</script>");
						con.close();
					}
				}
			catch (SQLException sExec) {
