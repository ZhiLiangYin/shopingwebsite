...
//執行 SQL 指令	
			if(request.getParameter("reset") != null && !request.getParameter("reset").equals("")){
				//sql = "SELECT `password` FROM `member` WHERE `name`=? AND `email`=?";
				PreparedStatement pstmt = null;
				//pstmt=con.prepareStatement(sql);
				//pstmt.setString(1,request.getParameter("name"));
				//pstmt.setString(2,request.getParameter("email"));
				//ResultSet rs = pstmt.executeQuery();
				sql="UPDATE `member` SET password=up654xu;6 WHERE `name`=? AND `email`=?";
				pstmt=con.prepareStatement(sql);
				pstmt.setString(1,request.getParameter("name"));
				pstmt.setString(2,request.getParameter("email"));
				int psw = pstmt.executeUpdate();
				if(psw>0){
					out.print("<script>alert('新密碼已寄到您的信箱!請重新登入變更密碼！');location.href='login.jsp'</script>");
				}
				else{
					out.print("<script>alert('資料有誤！請檢查後再試一次!');location.href='login.jsp'</script>");
				}
			}
			if(request.getParameter("name") != null && !request.getParameter("name").equals("") && request.getParameter("password") != null && !request.getParameter("password").equals("")){
				...
				if(rs.next()){            
					...
					sql = "SELECT `mid` FROM `member` WHERE `name`=? AND `password`=?";
					pstmt=con.prepareStatement(sql);
					pstmt.setString(1,request.getParameter("name"));
					pstmt.setString(2,request.getParameter("password"));
					rs = pstmt.executeQuery();
					rs.next();
					umid = rs.getInt(1);
					session.setAttribute("mid",umid);
					con.close();//結束資料庫連結
					out.print("<script>alert('登入成功！');location.href='member.jsp'</script>");
				}
				else{
					out.println("<script>alert('輸入的資料有誤！');location.href='login.jsp'</script>");
					con.close();
				} 
			}
