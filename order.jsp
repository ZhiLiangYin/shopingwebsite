...
<body>
    <header>
      ...
      <%
        ...
        <%
		if(request.getParameter("do_order")==null || request.getParameter("do_order").equals("")){
			out.print("<script>alert('請先登入，才能購物！');location.href='login.jsp'</script>");
		}
		String umid ="";
		...
		String pay_way=request.getParameter("sameadr");
		String name=request.getParameter("name");
		String tel=request.getParameter("tel");
		String pay_name=request.getParameter("cardname")+"";
		String pay_card=request.getParameter("cardnumber")+"";
		String address=request.getParameter("address");
		
		
		int i=0;
		String oid="";
		String cartAry[]= new String[]{"", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", ""};
		...
		//執行 SQL 指令
						// 寫入order訂單主檔案
						if(pay_way.equals("宅配")){
							sql="INSERT `order` (`mid`,`name`,`tel`,`pay_way`,`pay_name`,`pay_card`,`address`,`total`,`order_date`,`ship_status`) VALUES("+umid+",'"+name+"','"+tel+"','"+pay_way+"','"+pay_name+"','"+pay_card+"','"+address+"',"+total+",'"+ft.format(mydate)+"','已出貨')";
							
						}
						else{
							sql="INSERT `order` (`mid`,`name`,`tel`,`pay_way`,`address`,`total`,`order_date`,`ship_status`) VALUES("+umid+",'"+name+"','"+tel+"','"+pay_way+"','"+address+"',"+total+",'"+ft.format(mydate)+"','已出貨')";
						...
						//取得訂單單號 by mid
						sql="SELECT MAX(oid) FROM `order` WHERE `mid`='"+umid+"'";
						ResultSet rs=  con.createStatement().executeQuery(sql);
						rs.next();
						oid=rs.getString(1);
						...
						//轉出貨明細: cart轉ship
						sql="SELECT pid,price,amount,count FROM cart_view WHERE `status`='on' AND mid="+umid+"  ORDER by pid";
						...
						while(rs.next()){
							...
							db_amount=rs.getInt(3);//數量
							db_count=rs.getInt(4);//庫存
							if(db_amount>db_count || db_count==0)
									db_amount=db_count; //庫存=0 修正數量
								
							//out.print(db_pid+", "+db_price+", "+db_amount+", "+db_count+"<br> ");
							if(db_amount>0) {
								cartAry[i]="INSERT `ship` VALUES ("+oid+","+db_pid+","+db_amount+","+db_price+")"; // 轉出貨明細SQL
						...
						// 清購物車 cart by mid
						sql="DELETE FROM `cart` WHERE mid=" + umid;
						//out.println("清購物車 cart by umid: "+sql+"<br>");
						con.createStatement().executeUpdate(sql);
						out.print("<script>alert('Thank You!付款成功!祝您購物愉快!可至會員區查看訂單！');location.href='member.jsp'</script>");
					}
				}
			catch (SQLException sExec) {
				   out.println("SQL錯誤"+sExec.toString());
